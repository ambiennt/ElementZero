...
void checkInventoryTransaction(Mod::PlayerEntry const &, ComplexInventoryTransaction const &, Mod::CallbackToken<std::string> &);
...
void PreInit() {
	...
	Mod::AuditSystem::GetInstance().AddListener(SIG("inventory_transaction"), {Mod::RecursiveEventHandlerAdaptor(checkInventoryTransaction)});
	Mod::AuditSystem::GetInstance().AddListener(SIG("item_frame_drop"), {Mod::RecursiveEventHandlerAdaptor(checkItemFrameDrop)});
}
...
static bool Check(Player *player, int x, int z) {
	...
	if (player->isOperator() && mode == Mode::Permissive) return true;
	return false;
}

void checkAction(Mod::PlayerEntry const &entry, Mod::PlayerAction const &action, Mod::CallbackToken<std::string> &token) {
	DEF_LOGGER("debug");
...
void checkInventoryTransaction(Mod::PlayerEntry const &entry, ComplexInventoryTransaction const &transaction, Mod::CallbackToken<std::string> &token) {
	if (entry.player->getDimensionId() == 0) {
		...
		case ComplexInventoryTransaction::Type::ITEM_USE: {
			...
			case ItemUseInventoryTransaction::ActionType::PLACE:
				if (!Check(entry.player, data.pos.x, data.pos.z)) {
					const auto& legacy = *(entry.player->mRegion->getBlock(data.pos).mLegacyBlock.get());
