...
static RegisterQueue queue("timer", [](JsObjectWrapper global) {
  global["setTimeout"] = [](JsValueRef callee, Arguments args) -> JsValueRef {
    if (args.size() != 1 && args.size() != 2) throw std::runtime_error{"Requires 1 or 2 arguments"};
    auto fn = args[0];
    if (GetJsType(fn) != JsFunction) throw std::runtime_error{"First argument must be a function"};
